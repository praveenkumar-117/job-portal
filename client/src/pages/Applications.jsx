            ...
            isEdit ?
              <>
                <label htmlFor="resumeUpload" className='flex items-center'>
                  ...
                  <input onChange={e => setResume(e.target.files[0])} id='resumeUpload' accept='application/pdf' type="file" hidden />
        ...
        <table className='min-w-full bg-white border rounded-lg'>
          ...
          <tbody>
            {jobsApplied.map((job, index) => true ? (
              <tr>
                <td className='py-3 px-4 flex items-centergap-2 border-b'>
                ...
                <td className='py-2 px-4 border-b'>
                  <span className={`${job.status === 'Accepted' ? 'bg-green-100' : job.status === 'Rejected' ? 'bg-red-100' : 'bg-blue-100'} px-4 py-1.5 border rounded`}> 
                    {job.status}
                  </span>
                  </td>
              </tr>
